...
No *remover(Lista *lista, char string[])
{
    ...
    if (lista->inicio)
    {
        ...
        { // significa que possuo somente um elemento e nao preciso de comparacoes
            remover = lista->inicio;
            lista->inicio = NULL;
            lista->fim = NULL;
            lista->tam--;
        }
        else if ((strcmp(string, lista->inicio->nome)) == 0)
        { // significa que ele é o primeiro mas nao que a lista so possui um elemento
            remover = lista->inicio;
